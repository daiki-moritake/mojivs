...
Sequence / 異体字）** と **SVS（Standardized Variation Sequence / 標準異体字）** を、
**フォント自身の cmap format-14（UVS）サブテーブル**から解決します。これは OpenType
標準の異体字格納方式なので、Adobe-Japan1 に限らず、Hanyo-Denshi / Moji_Joho など他の
IVD コレクションや、CID-keyed / name-keyed いずれのフォントにも対応します（フォントが
format-14 を持たない場合は Adobe-Japan1 IVD へフォールバック）。Pillow の `ImageFont`
は既定（BASIC レイアウト）では異体字セレクタを無視します（libraqm/HarfBuzz を入れれば
描けます）。**mojivs は libraqm/HarfBuzz なしで、fonttools だけで IVS/SVS を解決します。**
