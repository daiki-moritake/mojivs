...
```bash
uv pip install "mojivs[cairo]"        # 既存の環境へインストール
uv add "mojivs[cairo]"                # uv 管理プロジェクトに追加
```
...
# 6. FreeType バックエンド（任意・cairo 非依存で高速。要 pip install mojivs[freetype]）
...
`--direction` `--align` `--orientation` `--tate-chu-yoko` `--backend`（`cairo`｜`freetype`）
