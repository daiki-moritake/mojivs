...
- **依存が少ない** — `fonttools` / `pillow` / `pycairo` / `numpy` のみ（PDF出力のみ `reportlab` が追加で必要）
...
（`size`・`direction`・`align`・`line_spacing`・`letter_spacing`・`padding`）と
...
- 縦書きは **glyph の回転を行いません**（欧文や一部記号は正立のまま）。CJK 本文向けの実装です。
